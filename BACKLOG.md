...
**Not verified live** — no Postgres/PostGIS available in this environment, so `npm run loadtest:matching` hasn't actually been run against the rewritten query. The fix follows the exact pattern the original finding described and reuses the existing `cleaner_home_gix` GiST index, but confirming the plan really shows an index scan instead of `Seq Scan on cleaner_profiles` is the next thing to do wherever Postgres is reachable.
