...
**Not done yet** — the rest of `render.yaml`'s `sync: false` placeholders are still unset on both services: `CHECKR_API_KEY`, `GOOGLE_MAPS_KEY`, `TWILIO_*`, `POSTMARK_TOKEN`/`MAIL_FROM`, `AWS_*`/`S3_BUCKET`, and `APP_URL` (no custom domain registered yet — Phase 1's other open item). `PAYPAL_ENV` is set to `live`, not `sandbox`; `render.yaml`'s own top-of-file note says to confirm a full booking/payment/payout cycle before flipping it, which hasn't happened against this deployment yet. `render.yaml`'s note 4 (the AWS SDK "known blocker") is now stale and should be removed the next time that file is touched.
