...
Same-origin by default. To point at an API on another origin, set `window.__SPARKLE_API_BASE__` (e.g. `"http://localhost:8080/v1"`) before the bundle loads; the backend already reflects CORS with credentials. `AdminConsole.jsx` and `api.js` are the two files to drop into a host app (React + Tailwind + `lucide-react`) — but this directory also carries its own Vite scaffold (`index.html`, `main.jsx`, `index.css`, `vite.config.js`) for running it standalone with `npm run dev`. That dev server proxies same-origin `/v1` calls (`ADMIN_API_PROXY_TARGET`, default `http://localhost:8080`) so `api.js`'s default same-origin `BASE` just works without setting `window.__SPARKLE_API_BASE__`.
