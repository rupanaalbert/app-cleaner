...
- **`cleaner_app`'s Discover tab confirmed rendering on the VPS (2026-09-11)** (real empty-state, real "Accepting jobs" toggle) but Schedule/Earnings/Approval weren't reachable this session — the VPS's AVD is unaccelerated (no nested-virtualization `/dev/kvm` despite the "KVM VPS" product name) with only ~2.5GB RAM, which reliably triggers an ANR in Geolocator's GNSS-assist-data fetch on cold launch and can leave WindowManager's input focus stuck on the dismissed ANR dialog afterward (recovers with force-stop + relaunch). This is an environment limitation, not an app bug — see BACKLOG.md item 8 for the full pattern.
