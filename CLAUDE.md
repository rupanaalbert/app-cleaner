...
- **Real Firebase project set up on this machine (2026-08-28), closing the gap below.** Project `sparkle-cleaning` (`sparkle-cleaning-d7485`), Spark (free) plan, Realtime Database in `us-central1`, Authentication initialized, both Android apps registered. `backend/secrets/firebase.json` (the Admin SDK key) and both apps' `android/app/google-services.json` are real and gitignored — regenerate them from the Firebase console (Project Settings → Service accounts / → your apps) if this machine's copies are lost; they're not recoverable from git. Two real bugs surfaced getting this working, both fixed: (1) `infra/firebase/database.rules.json`'s `"//": "..."` was never valid Firebase rules — a rule node must be an object, not a string; real comments need bare `//` syntax, which the file now uses. (2) `realtime.service.js`/`notification.service.js` called `credential: applicationDefault()`, which reads `GOOGLE_APPLICATION_CREDENTIALS` (never set here), not the path in `FIREBASE_SERVICE_ACCOUNT_JSON` — fixed to `cert(JSON.parse(readFileSync(...)))`, matching Firebase's own generated snippet. `FIREBASE_DATABASE_URL` was also missing from `.env.example` entirely; both env vars are required together. Verified live: `PATCH .../status` to `arrived` and `in_progress` both return `200` against the real backend now (previously `500`), and the Firebase console's Data tab shows a real `booking_access/{bookingId}` node. Full detail, including why the console's rules editor had to be bypassed in favor of a REST `PUT`, in BACKLOG.md.
...
See `BACKLOG.md` — all twelve backlog items are done except item 11 (Render deployment, in progress). Device-pass work has moved to a Hostinger VPS (`infra/vps-setup.sh`) since this machine's local Flutter/emulator work hit Dart VM OOM issues; see BACKLOG.md item 8's 2026-09-11 entry for the VPS's current state (backend/worker/admin running there in detached `screen` sessions) before starting a fresh session there. Remaining gaps worth picking up next:

- **`customer_app`'s full booking flow verified interactively on the VPS (2026-09-11)**, all the way through a real PayPal sandbox order creation — hero banner, service selection, property details, `RadioGroup` date/time picker, and `review_step.dart` (a Phase 3 screen with no prior device pass) all confirmed live, with a real computed price ($154.49) and a real `201` from both `/v1/quotes` and `/v1/quotes/:id/paypal-order`. Found and worked around a real dev-config gap along the way: `main.dart`'s hardcoded `propertyId` is tied to whichever machine's `npm run seed` produced it, so it 404s on any other independently-seeded database (VPS included) — the long-term fix is still a real property picker, not a hardcoded id. **New blocker found:** tapping through to PayPal's actual "Approve with PayPal" WebView crashes the whole app on this VPS (Chromium renderer crash, not a network issue) — likely specific to the VPS's stripped-down `aosp_atd` system image rather than a Sparkle bug, since this exact flow was previously verified working on a different emulator. See BACKLOG.md item 8 for the full trace and a next step (try a `google_apis` AVD instead of `aosp_atd`).
- **`cleaner_app`'s Discover tab confirmed rendering on the VPS (2026-09-11)** (real empty-state, real "Accepting jobs" toggle) but Schedule/Earnings/Approval weren't reachable this session — every one of five cold launches ANR'd in Geolocator's foreground-service bind (~60-70s) and left WindowManager's input focus permanently stuck afterward. Unlike `customer_app` (where force-stop + relaunch reliably recovered from its one ANR), the same recovery was tried five different ways for `cleaner_app` and none worked. This is an environment limitation (unaccelerated VPS, no `/dev/kvm`, ~2.5GB RAM), not an app bug — see BACKLOG.md item 8 for the full pattern and what to try next (a `google_apis` AVD, more RAM, or real hardware acceleration).
