...
- **`customer_app`'s booking flow verified interactively on the VPS (2026-09-11)** as far as the scheduling step — hero banner, service selection, property details, and the `RadioGroup` date/time picker all confirmed live. Found and worked around a real dev-config gap: `main.dart`'s hardcoded `propertyId` is tied to whichever machine's `npm run seed` produced it, so it 404s on any other independently-seeded database (VPS included) — the long-term fix is still a real property picker, not a hardcoded id. **Not yet reached:** the review step, an actual `Review booking` submission, and any chat/tracking screens.
