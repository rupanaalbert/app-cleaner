...
- The PayPal integration (`payment.service.js`, `webhook.service.js`, `onboarding.service.js`) is built against PayPal's documented API shapes but hasn't been exercised against a live sandbox — `PAYMENT.CAPTURE.REFUNDED`/`CUSTOMER.DISPUTE.CREATED`'s exact resource payload, partial-capture behavior, and the Payouts feature's enablement process on the live account all need a real sandbox pass before this goes anywhere near production traffic. Do that spike first; correct the handlers if reality differs.
